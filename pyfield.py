...
class Field:
    def __init__(self, *argv):
        if (type(argv[0]) == np.ndarray):
            ndim = len(argv[0].shape)
            if (ndim == 2) and (type(argv[1]) == float):
                self.dim = np.array(argv[0].shape)
                self.search_charges(argv[0],argv[1])

            if (ndim == 1) and (type(argv[1]) == np.ndarray):
                self.set_charges(argv[0],argv[1])
    ...
    def fline(self, x, maxi = 200, step = 0.5):
        ...
        f2 = self.rk2(x, mul = -1, maxi = maxi, step = step)
        if f1[1] != f2[1]:
